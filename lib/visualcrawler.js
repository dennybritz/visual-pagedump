  ...
  getPageData(client) {
    return Promise.resolve(client.elements('*')).get('value').map(jsonWebElement => {
      ...
      return Promise.all([
        ...
        Promise.resolve(client.elementIdSize(elementID)).get('value').then(obj => _.pick(obj, 'width', 'height')),
        Promise.resolve(client.elementIdLocation(elementID)).get('value').then(obj => _.pick(obj, 'x', 'y')),
